...
Heroku ya nos facilitaba herramientas para desplegar el código directamente al servidor utilizando \textit{git}, pero como tuvimos que abandonarlo, en un \textit{droplet} no tenemos esta facilidad.
...
El primer problema encontrado fue que estos arquivos \textit{GeoJSON} eran demasiado grandes. La solución a este problema fue utilizar un herramienta, \foothref{mapshaper}{http://mapshaper.org/}. Con esta herramienta se consiguió suavizar los bordes de las regiones minimizando el tamaño de los archivos \textit{GeoJSON}.
